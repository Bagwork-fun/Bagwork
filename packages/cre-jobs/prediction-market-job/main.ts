...
//  - Gemini key for local simulate: `simulationGeminiApiKey` in `config.sim-run.json` (written by scripts); optional
//    `process.env` / Vault fallback via runtime `getSecret` on newer CLIs.
...
// Local simulate reads `simulationGeminiApiKey` from `config.sim-run.json` (generated by npm scripts).
// Optional fallbacks: host env or Vault (`getSecret`) on newer CRE CLI builds.
// ======================================================

type SecretGetter = (req: { id: string; namespace?: string }) => { result: () => { value: string } };

/** Simulation scripts merge GEMINI into config; production uses Vault via optional `getSecret`. */
function resolveGeminiApiKey(nodeRuntime: NodeRuntime<Config>): string {
  const fromCfg = String(nodeRuntime.config.simulationGeminiApiKey || "").trim();
  if (fromCfg) return fromCfg;

  try {
    const env =
      typeof globalThis.process !== "undefined" && globalThis.process.env ? globalThis.process.env : undefined;
    if (env) {
      const k =
        String(env.GEMINI_API_KEY || "").trim() ||
        String(env.GOOGLE_API_KEY || "").trim() ||
        String(env.GEMINI_KEY || "").trim();
      if (k) return k;
    }
  } catch {
    /* ignore */
  }
  const impl = nodeRuntime as NodeRuntime<Config> & { getSecret?: SecretGetter };
  if (typeof impl.getSecret === "function") {
    return impl.getSecret.call(nodeRuntime, { id: "GEMINI_API_KEY", namespace: "main" }).result().value;
  }
  throw new Error(
    "No Gemini API key: add GEMINI_API_KEY to layered .env and run via simulate-resolve / npm run simulate " +
      "(which writes config.sim-run.json), or configure Vault secrets for deployment."
  );
}

const askGeminiOnNode = (nodeRuntime: NodeRuntime<Config>, metadata: MarketMetadata): GeminiResponse => {
  const geminiApiKey = resolveGeminiApiKey(nodeRuntime);
  const httpClient = new HTTPClient();
...
const onLogTrigger = (runtime: Runtime<Config>, log: EVMLog): string => {
  try {
    ...
    const geminiResult: GeminiResponse = runtime
      .runInNodeMode(
        (nodeRuntime: NodeRuntime<Config>) =>
          askGeminiOnNode(nodeRuntime, metadata),
    ...
    runtime.log(`🔗 View: https://testnet.arcscan.app/tx/${txHash}`);
