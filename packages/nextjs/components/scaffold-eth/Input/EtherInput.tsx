...
export const EtherInput = ({
  ...
  return (
    <InputBase
      ...
      disabled={disabled}
      prefix={<span className="pl-4 -mr-2 text-muted-foreground self-center font-medium">{displayUsdMode ? "$" : "Ξ"}</span>}
      suffix={
        <div
          className="flex shrink-0 pr-1"
          title={
            nativeCurrencyPrice > 0 ? undefined : isNativeCurrencyPriceFetching ? "Fetching price" : "Unable to fetch price"
          }
        >
          <Button
            variant="secondary"
            size="icon-sm"
            className="h-[2.2rem] w-8 shrink-0 rounded-full"
